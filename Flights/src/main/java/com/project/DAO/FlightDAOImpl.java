...
public List<Flights> getReturnFlightsAvailable(String source, String destination, LocalDate traveldate, LocalDate returndate, int seats)
{
	Query query = em.createQuery("select fs from FlightEntity fs where fs.source=?1 and fs.destination=?2 and fs.traveldate=?3 and fs.seatsavailable>=?4");
	query.setParameter(1, source);
	query.setParameter(2, destination);
	...
	Query query1=em.createQuery("select fs from FlightEntity fs where fs.source=?1 and fs.destination=?2 and fs.traveldate=?3 and fs.seatsavailable>=?4" );
	query1.setParameter(1, destination);
	query1.setParameter(2, source);
	query1.setParameter(3, returndate);
	query1.setParameter(4, seats);
	List<FlightEntity> entitylist1 = query1.getResultList();
	if(!entitylist1.isEmpty())
	{
		entitylist1.stream().forEach(n->{
			Flights f = new Flights();
			f.setDestination(n.getDestination());
			f.setFlightno(n.getFlightno());
			f.setSeatsavailable(n.getSeatsavailable());
