...
public ResponseEntity<List<Flights>> getReturnFlightsAvailable(@PathVariable String source,@PathVariable String destination,@PathVariable String traveldate,@PathVariable String returndate,@PathVariable String seatsRequired)
{
	ResponseEntity<List<Flights>> response = null;
	try {
		List<Flights> list = fs.getReturnFlightsAvailable(source, destination, traveldate, returndate, seatsRequired);
		response=new ResponseEntity<List<Flights>>(list,HttpStatus.OK);
	}
	catch(Exception e)
	{
